import { InMemoryAdminStorage } from './InMemoryAdminStorage';
...
describe('Admin Users', () => {
  ...
  it('createAdminUser returns user with password_hash present (deep clone)', async () => {
    ...
    // The returned object is a deep clone that includes password_hash
    // (The API layer is responsible for stripping it before sending to clients)
  ...
  it('validatePassword returns null for disabled user', async () => {
    await storage.disableAdminUser('admin', 'system');
    const user = await storage.validatePassword('admin', 'changeme');
    expect(user).toBeNull();
  });
  ...
  it('enableAdminUser sets enabled=true', async () => {
    await storage.disableAdminUser('admin', 'system');
    await storage.enableAdminUser('admin', 'system');
    const user = await storage.getAdminUser('admin');
    expect(user!.enabled).toBe(true);
  ...
  it('setAdminUserPassword changes the password', async () => {
    ...
    await storage.setAdminUserPassword('admin', newHash, 'admin');

    // Old password should fail
    const oldResult = await storage.validatePassword('admin', 'changeme');
    expect(oldResult).toBeNull();

    // New password should work
    const newResult = await storage.validatePassword('admin', 'newpass123');
  ...
  it('validatePassword locks account after 5 failed attempts', async () => {
    for (let i = 0; i < 5; i++) {
      await storage.validatePassword('admin', 'wrongpassword');
    }
    // Even correct password should now fail due to lock
    const user = await storage.validatePassword('admin', 'changeme');
    expect(user).toBeNull();
  });
});
...
describe('Client Management', () => {
  ...
  it('listClients with status filter', async () => {
    await storage.registerClient({ hostname: 'a.example.com' });
    const client2 = await storage.registerClient({ hostname: 'b.example.com' });
  ...
  it('listClients with hostname search', async () => {
    await storage.registerClient({ hostname: 'alpha.example.com' });
    await storage.registerClient({ hostname: 'beta.example.com' });

    const result = await storage.listClients({ hostname: 'alpha' });
    expect(result.clients.length).toBe(1);
    expect(result.clients[0]!.hostname).toBe('alpha.example.com');
  });

  it('listClients with pagination', async () => {
    for (let i = 0; i < 5; i++) {
      await storage.registerClient({ hostname: `host${i}.example.com` });
    }
    const page1 = await storage.listClients({ limit: 2, offset: 0 });
    expect(page1.clients.length).toBe(2);
    expect(page1.total).toBe(5);

    const page2 = await storage.listClients({ limit: 2, offset: 2 });
    expect(page2.clients.length).toBe(2);
    expect(page2.offset).toBe(2);
  });

  it('getPendingClients returns only pending clients', async () => {
    const client1 = await storage.registerClient({ hostname: 'p1.example.com' });
    await storage.registerClient({ hostname: 'p2.example.com' });
    ...
    expect(pending.some(c => c.hostname === 'p2.example.com')).toBe(true);
    expect(pending.some(c => c.hostname === 'p1.example.com')).toBe(false);
  });
...
describe('Audit', () => {
  ...
  it('getAuditLog pagination works', async () => {
    ...
    const page1 = await storage.getAuditLog({ limit: 2, offset: 0 });
    expect(page1.entries.length).toBe(2);
    expect(page1.total).toBeGreaterThanOrEqual(5);

    const page2 = await storage.getAuditLog({ limit: 2, offset: 2 });
